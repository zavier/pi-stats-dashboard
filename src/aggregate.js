...
export async function aggregate(root=sessionsRoot, now=Date.now()) {
  ...
  const files=await walk(root), seen=new Set(), seenTools=new Set(), all=zero(), ranges={all:zero(),today:zero(),week:zero(),month:zero()}, groups=dims(), rangeGroups=Object.fromEntries(rangeNames.map(n=>[n,dims()])), days=new Map(), rangeDays=Object.fromEntries(rangeNames.map(n=>[n,new Map()])), daysSeries=new Map(), rangeDaysSeries=Object.fromEntries(rangeNames.map(n=>[n,new Map()])), behavior={messages:0,chars:0,words:0,yelling:0,profanity:0,anguish:0,negation:0,repetition:0,blame:0}, diagnostics={files:files.length,invalidLines:0,skippedRecords:0,standardSessions:0,transcriptFiles:0};
  ...
  function request(m, meta) { const ts=Number(m.timestamp)||meta.timestamp||0, u=m.usage; if(!u || typeof u!=="object") {diagnostics.skippedRecords++;return} const sig=idHash({type:"message",id:meta.id,timestamp:ts,message:{usage:u}}); if(seen.has(sig))return; seen.add(sig); const error=m.stopReason==="error", amount=(+u.input||0)+(+u.output||0)+(+u.reasoning||0)+(+u.cacheRead||0)+(+u.cacheWrite||0); add(all,u,error); const groupKeys={model:key(m),provider:m.provider??"unknown",project:meta.project,agent:meta.agent}; addGroups(groups,groupKeys,u,error); const day=new Date(ts).toISOString().slice(0,10), d=days.get(day)||zero();add(d,u,error);days.set(day,d);bumpSeries(daysSeries,day,groupKeys.model,groupKeys.provider,amount);for(const [name,cut] of Object.entries(cutoffs))if(ts>=cut){add(ranges[name],u,error);addGroups(rangeGroups[name],groupKeys,u,error);const rd=rangeDays[name].get(day)||zero();add(rd,u,error);rangeDays[name].set(day,rd);bumpSeries(rangeDaysSeries[name],day,groupKeys.model,groupKeys.provider,amount);} }
