...
function projectOf(file, root=sessionsRoot) { const slug=relative(root,file).split("/")[0]; if(!slug) return "unknown"; const decoded=slug.replace(/^-+/,"/").replace(/-/g,"/").replace(/\/+$/,""), base=decoded.split("/").filter(Boolean).pop()||slug; return `${base} · ${createHash("sha1").update(decoded).digest("hex").slice(0,6)}`; }
...
export async function aggregate(root=sessionsRoot, now=Date.now()) {
  ...
  for(const file of files) { let text; try { text=await readFile(file,"utf8"); } catch { diagnostics.unreadableFiles++; continue; } const standard=text.split("\n").some(l=>{try{return JSON.parse(l).type==="session"}catch{return false}}); if(standard) diagnostics.standardSessions++; else diagnostics.transcriptFiles++;
    let lastModel={}; for(const line of text.split("\n")){if(!line.trim())continue;let x;try{x=JSON.parse(line)}catch{diagnostics.invalidLines++;continue} const meta={id:x.id, timestamp:Date.parse(x.timestamp)||0, project:projectOf(file,root), agent:standard?"main":basename(file).split("_")[0]||"subagent"};
